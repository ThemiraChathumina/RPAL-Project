...
void print_exec_node(exec_node *in_element){
	...
	switch(in_element->type){
		case EXEC_INT: cout<<in_element->int_val;
			  	break;
		...
		case EXEC_STR: //printf("%c", (in_element->str_val).c_str());
			      ...
			      break;
		case EXEC_ENV: cout<<"<ENVIRONMENT>"<<endl;
			      break;

		case EXEC_TUPPLE: //cout<<"<PRINTING TUPPLE>"<<endl;
				 print_tuple(in_element);
			      break;
		case EXEC_CTRL_STRUCT: cout<<"[lambda closure: ";
				      cout<<in_element->ctrl_node_val->bound_variable->value<<" ]";
			      break;
		case EXEC_TRUE: cout<<"true";
			       break;

		case EXEC_FALSE: cout<<"false";
			       break;

		case EXEC_DUMMY: cout<<"dummy";
			       break;
		case EXEC_YSTAR: cout<<"<YSTAR>"<<endl;
			       break;

		case EXEC_NILL: cout<<"nil";
			       break;
		case EXEC_ITA: cout<<"<ITA>"<<endl;
				cout<<"[lambda closure: ";
...
void execute(){
	...
	while( control_stack.empty() != true ){
		...
		//apply rules
		if( popped_cntrl_st->node->type == INT){ //if node is INTEGER
		...
		else if( popped_cntrl_st->node->type == STR){ //if node is STRING 
		...
		//if node is IDENTIFIER
			...
			if( idt_exe_ele == NULL){
				...
				else if( ((popped_cntrl_st->node->value).compare("Stern") == 0 ) || ((popped_cntrl_st->node->value).compare("stern") == 0)
						|| ((popped_cntrl_st->node->value).compare("Stem") == 0) || ((popped_cntrl_st->node->value).compare("stem") == 0)
					||((popped_cntrl_st->node->value).compare("Conc") == 0) || ((popped_cntrl_st->node->value).compare("conc") == 0))
				{
					//do appropriate action
					//create new execution element
					exec_node *new_exe_ele = new exec_node;
		...
		else if (popped_cntrl_st->node->type == LAMBDA){ //if node is LAMBDA
		...
		else if(popped_cntrl_st->node->type==TAU){ //if node is TAU
		...
		/* TODO - Remember to update the current environment */
		else if(popped_cntrl_st->node->type == ENV) { //if it is ENVIRONMENT
			...
			if(saved_exe_ele != NULL){
				...
				if( !( (another_exe_ele->type == EXEC_ENV) && ( (another_exe_ele->env_val) ==  (popped_cntrl_st->env) ) )){ //confirm if it is environment and same as the popped one
		...
		//Handle <YSTAR> node
		...
		else if(popped_cntrl_st->node->type == GAMMA){ //if node is GAMMA
			//check the element on execution stack
			if( (exec_stack.top())->type == EXEC_CTRL_STRUCT ) { //there is lambda on execution stack - RULE 3
			...
			//must be some primitive function
				...
				//CONC needs two gamma
					...
					if(stern_element== NULL || stern_element->type != EXEC_STR){ //should be string
			...
			else if((exec_stack.top())->type == EXEC_TUPPLE){ //if there is TUPPLE on Execution Stack
				//pop the tupple
				...
				if(second_exe_ele == NULL || second_exe_ele->type != EXEC_INT){ //should be INTEGER only
		...
		else if( (popped_cntrl_st->node->type == BETA) ) { //BETA Node
		...
		else if(popped_cntrl_st->node->type == EQ
				|| popped_cntrl_st->node->type == NE){
			//pop two element
			...
			else if(temp_first->type == EXEC_INT && temp_second->type == EXEC_INT){ //both are INTEGERS
		...
		//PLUS, DIV operator
				...
				(popped_cntrl_st->node->type == MULTI)
				|| (popped_cntrl_st->node->type == LS) )  {
			...
			if (popped_cntrl_st->node->type == GR || (popped_cntrl_st->node->type == LE)
					||(popped_cntrl_st->node->type == LS)){
				if (temp_result)
